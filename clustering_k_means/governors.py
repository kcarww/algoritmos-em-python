...
if __name__ == '__main__':
    governors: List[Governor] = [
        ...
        Governor(-80.954067, 66, "West Virginia"),
    ...
    for idx, cluster in enumerate(gov_clusters):
        xs = [governor.longitude for governor in cluster.points]
        ys = [governor.age for governor in cluster.points]
        plt.scatter(xs, ys, color=cores[idx % len(cores)], label=f'Cluster {idx}')
        for governor in cluster.points:
            plt.annotate(governor.state, (governor.longitude, governor.age),
                         textcoords="offset points", xytext=(5,5), fontsize=8)

    plt.xlabel("Longitude")
    plt.ylabel("Idade")
    plt.title("Clusters de Governadores")
    plt.legend()
    plt.grid(True)
    plt.show()
